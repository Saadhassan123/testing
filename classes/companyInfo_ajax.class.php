...
class companyInfo_ajax extends object_class{
...
public function deleteInfo(){
    try{
        ...
        $sql3="SELECT `logo` FROM `company_info` WHERE id='$id'";
        $data=$this->dbF->getRow($sql3,false);

        @unlink(__DIR__."/../../../images/$data[logo]");

        $del_sql = " DELETE FROM `company_info` WHERE `id` = ? ";
